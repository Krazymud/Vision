#include "blend.h"
Blend::Blend(CImg<unsigned char>& a, CImg<unsigned char>& b){
    this->a = a;
    this->b = b;
    blending();
}

bool Blend::isZero(CImg<unsigned char> &img, int x, int y) {
	return (img(x, y, 0) == 0 && img(x, y, 1) == 0 && img(x, y, 2) == 0);
}

CImg<unsigned char> Blend::returnRes(){
    return res;
}
...
void Blend::blending() {
	...
	if (a.width() > a.height()) {
		for (int x = 0; x < a.width(); x++) {
			if (!isZero(a, x, a.height() / 2)) {
				sum_a_x += x;
				a_n++;
			}

			if (!isZero(b, x, b.height() / 2)) {
				sum_b_x += x;
				b_n++;
			}

			if (!isZero(a, x, a.height() / 2) && !isZero(b, x, a.height() / 2)) {
				sum_overlap_x += x;
				overlap_n++;
			}
		}
		...
		for (int y = 0; y < a.height(); y++) {
			if (!isZero(a, a.width() / 2, y)) {
				sum_a_y += y;
				a_n++;
			}

			if (!isZero(a, a.width() / 2, y) && !isZero(b, b.width() / 2, y)) {
				sum_overlap_y += y;
				overlap_n++;
			}
		}
